...
def pg_employees():
    ...
    if ok:
        ...
        if not errs:
            try:
                qrun("INSERT INTO users(full_name,username,email,password,role) VALUES(?,?,?,?,?)",(fn.strip(),un.strip().lower(),em.strip().lower(),_hash(p1),rl))
    ...
    if all_u:
        ...
        uid_t=opts[sel]; ca,cb,_=st.columns([1,1,3])
